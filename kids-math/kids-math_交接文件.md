...
| 4 | 25 | 50 |
| 5 | 50 | 100 |
- Lv1–2 固定顯示圖像輔助；Lv3–5 固定為純算式
- Lv3 為 20 以內不進位加法；Lv4 起加入進位
...
## 十二、2026-06-27 形狀稱呼與易混淆選項修正

### 稱呼更新

- `星形` 改為幼兒常用稱呼 `星星`，注音改為 `ㄒㄧㄥ ㄒㄧㄥ`。
- `心形` 改為幼兒常用稱呼 `愛心`，注音改為 `ㄞˋ ㄒㄧㄣ`。
- `shapes.html` 的答案、選項及音檔對照已更新。
- `learn-shapes.html` 的星星名稱與注音已同步更新。

### 易混淆選項群組

形狀題產生干擾選項時，同組名稱不會同時出現：
...
- 顏色題不受影響。
- 選項仍維持純文字與注音，不加入圖示，避免題目退化成圖形配對。

### 待補語音

- 已新增 `kids-math_shapes稱呼更新語音移交4.csv`。
- 請用 TTS 重新產生並覆蓋：
  - `audio/shapes/star.mp3`：星星
  - `audio/shapes/heart.mp3`：愛心
- 在新檔覆蓋前，程式路徑仍可播放舊音檔，但台詞會暫時與新畫面稱呼不同。

---

## 十三、2026-06-27 五關卡難度曲線統一

### 共通升級規則

- 五個練習模式的升級門檻由 `[5,10,20,30]` 改為 `[5,8,12,15]`。
- 從 Lv1 到 Lv5 共需答對約 40 題，取代原本的 65 題。
- 答錯仍不降級、不扣除已累積經驗，只會中斷連答顯示。
- 挑戰模式原本的「🚀 自動」改名為「🚀 漸進挑戰」，明確表示它是每 5 題提升等級，而非依答對率即時調整。

### 數數字

- 選項數改為 `5 → 5 → 6 → 8 → 10`，避免 Lv2 直接跳到 10 個選項。
- 數量範圍維持 `5 → 10 → 20 → 50 → 100`。

### 加法

- Lv1–2 固定顯示 emoji 圖像；Lv3–5 固定隱藏圖像，不再依隨機數值忽有忽無。
- Lv3 範圍為 20 以內，並排除個位數進位題；Lv4 起才加入進位。
- Lv3–5 題目會完整朗讀「A 加 B 等於幾」，Lv1–2 維持情境句「加在一起是幾個」。
- 干擾答案限制在當前等級的結果範圍內。

### 減法

- Lv1–2 固定顯示拿走／變灰圖像；Lv3–5 固定為純算式。
- Lv3 範圍為 20 以內，並排除需要個位數退位的題目；Lv4 起才加入退位。
- Lv3–5 題目會完整朗讀「A 減 B 等於幾」，解決純算式只播放情境句的突兀感。
- 干擾答案限制在當前等級範圍內。

### 加減綜合

- Lv1–2 補上與單獨加減法相同的 emoji 圖像輔助。
- Lv3–5 固定為純算式，並完整朗讀算式。
- Lv3 同樣排除加法進位及減法退位；Lv4 起加入。
- 干擾答案限制在當前等級範圍內。

### 形狀

- 可用形狀數量調整為 `3 → 5 → 8 → 11 → 14`。
- 各級新增形狀改為 `+2 → +3 → +3 → +3`，取代 Lv2 一次增加 4 種的跳躍。
- 顏色題比例與易混淆選項群組維持不變。

### 驗證結果

- 執行 100,000 次隨機產題與選項抽樣，數值範圍、Lv3 不進位／不退位及干擾答案限制均通過。
- 形狀解鎖數量確認為 `3、5、8、11、14`。
- 修正五個遊戲頁全螢幕同步函式前的多餘字元；函式現在可在載入時正常註冊。

---

## 十四、2026-06-27 五個教學頁同步更新

### 內容修正

- `learn-counting.html` 更新為實際的 5、10、20、50、100 難度範圍，並明確說明答錯不會降級。
- `learn-addition.html` 移除「答錯會自動降級」的過時說明。
- `learn-shapes.html` 移除「Lv4–5 才加入顏色與旋轉」的過時說明，改為各級皆可能出現顏色題、形狀不旋轉。
- 修正形狀資料注音：`梯形` 改為 `ㄊㄧ ㄒㄧㄥˊ`、`橘色` 改為 `ㄐㄩˊ ㄙㄜˋ`。

### 形狀教學補齊

- 形狀教學由 9 種補齊為遊戲使用的 14 種。
- 新增：愛心、半圓形、梯形、平行四邊形、箭頭形。
- 每個形狀皆包含名稱、注音、生活例子、SVG 圖形及點擊語音。

### 語音與互動

- `learn-counting.html` 與 `learn-shapes.html` 補上 `currentAudio` 防疊音，五個教學頁現在皆使用單一音源控制。
- 加法、減法的概念語音按鈕移入概念卡，步驟語音按鈕移入示範卡。
- 綜合頁改為「聽加減法概念」及依目前題號播放的「聽這一題」。
- 綜合判斷題答錯後不再自動換題、不揭露答案；短錯誤音效播放完畢後解鎖同一題供重新選擇。
- 綜合判斷題答對後播放短音效，再進入下一題。

### 平板版面

- 五個教學頁新增橫向 900px 以上的雙欄排版，最大內容寬度 1100px。
- 加法、減法、數數字將概念與互動示範並排。
- 綜合頁將概念比較與判斷練習並排。
- 形狀頁在平板橫向使用五欄形狀卡片。
- 手機與平板直向仍維持原本單欄閱讀順序。
...
- 五個教學頁 JavaScript 語法與 CSS 結構檢查通過。
- 五頁皆確認包含防疊音與平板橫向規則。
- 14 種形狀及對應音檔路徑均已確認存在。
- 過時說明與綜合題答錯重選流程檢查通過。
